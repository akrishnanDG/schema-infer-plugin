...
class KafkaConsumer:
    ...
    def _initialize_consumer(self) -> None:
        ...
        try:
            ...
            self.logger.info(
                f"Initialized Kafka consumer with servers: {self.config.kafka.bootstrap_servers}"
            )
    ...
    def consume_topic(
        ...
        try:
            ...
            while message_count < max_messages:
                ...
                if value is not None:  # Only process non-null values
                    ...
                    if message_count % 100 == 0:
                        self.logger.debug(
                            f"Consumed {message_count} messages from {topic_name}"
                        )

            self.logger.info(
                f"Consumed {len(messages)} messages from topic {topic_name}"
            )
            ...
            self.logger.error(
                f"Unexpected error while consuming from {topic_name}: {e}"
            )
    ...
    def consume_topics(
        self, topic_names: List[str], max_messages_per_topic: int, timeout: int
