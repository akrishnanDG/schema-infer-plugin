...
class TopicDiscovery:
    ...
    def _should_exclude_topic(
        self, topic_name: str, exclude_internal: Optional[bool] = None
    ) -> bool:
        """
    ...
    def _discover_by_prefix(
        self, prefix: str, exclude_internal: Optional[bool] = None
    ) -> List[str]:
        """
        ...
        try:
            with KafkaConsumer(self.config) as consumer:
                ...
                self.logger.info(
                    f"Found {len(filtered_topics)} topics with prefix '{prefix}'"
                )
    ...
    def _discover_by_pattern(
        self, pattern: str, exclude_internal: Optional[bool] = None
    ) -> List[str]:
        """
        ...
        try:
            ...
            with KafkaConsumer(self.config) as consumer:
                ...
                self.logger.info(
                    f"Found {len(filtered_topics)} topics matching pattern '{pattern}'"
                )
                return filtered_topics
            ...
            with KafkaConsumer(self.config) as consumer:
                for topic_name in topic_names:
                    try:
                        ...
                        self.logger.warning(
                            f"Failed to get metadata for topic {topic_name}: {e}"
                        )
    ...
    def filter_topics_by_criteria(
                ...
                for topic_name in topics:
                    ...
                    try:
                        ...
                        if metadata.get("error"):
                            self.logger.warning(
                                f"Topic {topic_name} has errors: {metadata['error']}"
                            )
                            continue
        ...
        self.logger.info(
            f"Filtered {len(topics)} topics down to {len(filtered_topics)}"
        )
        return filtered_topics
                    ...
                    try:
                        ...
                        if metadata.get("error"):
                            results["inaccessible"].append(
                                {"topic": topic_name, "error": metadata["error"]}
                            )
                        else:
