...
class SchemaInferrer:
                ...
                try:
                    ...
                    if result["success"]:
                        ...
                        if result.get("multi_event"):
                            ...
                            event_types = list(
                                result["multi_event_data"]["event_schemas"].keys()
                            )
                            disc = result["multi_event_data"]["discriminator_field"]
                            print(
                                f"OK {result['topic']}: Multi-event ({len(event_types)} types via '{disc}') in {result['processing_time']:.2f}s ({result['message_count']} messages)"
                            )
                        else:
                            results["schemas"][result["topic"]] = result["schema"]
                            print(
                                f"OK {result['topic']}: Generated schema in {result['processing_time']:.2f}s ({result['message_count']} messages)"
                            )
                    else:
                        results["failed"] += 1
    ...
    def infer_schema(
        self, messages: List[Tuple[Optional[bytes], bytes]], topic_name: str
        ...
        try:
            ...
            if not message_values:
                ...
                print(
                    "⚠️  No valid message values found - all messages may be empty or corrupted"
                )
                return None

            self.logger.info(
                f"Processing {len(message_values)} messages for schema inference"
            )
            ...
            if self.config.inference.auto_detect_format:
                ...
                self.logger.info(
                    f"Detected format: {detected_format} (confidence: {confidence:.2f})"
                )
            else:
            ...
            if not parsed_data:
                ...
                print(
                    f"⚠️  Could not parse messages in {detected_format} format - trying fallback approach"
                )
                ...
                if detected_format != "raw-text":
                    ...
                    if parsed_data:
                        ...
                        all_binary = all(
                            ...
                            and record.get("is_binary", False)
                            and "raw_content" in record
                            for record in parsed_data
                        )

                        if all_binary:
                            print(
                                "❌ All messages are in binary format - schema cannot be inferred"
                            )
                            print(
                                "💡 Binary messages require specific deserializers (Avro, Protobuf, etc.)"
                            )
                            print(
                                "💡 Consider using Schema Inference Schema Registry with proper serializers"
                            )
                            return None
            ...
            schema_dict["_metadata"] = {
                ...
                "confidence": (
                    confidence if self.config.inference.auto_detect_format else 1.0
                ),
            }
    ...
    def infer_multi_event(
        ...
        try:
            ...
            if discriminator_field:
                ...
                discriminator_field = self.schema_analyzer.detect_discriminator(
                    parsed_data
                )
