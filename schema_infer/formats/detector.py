...
class FormatDetector:
    ...
    def detect_format(self, messages: List[bytes]) -> Tuple[str, float]:
        ...
        if not text_messages:
            ...
            self.logger.warning(
                "No valid text messages found - treating as binary/raw-text format"
            )
            return "raw-text", 0.1
        ...
        self.logger.info(
            f"Detected format: {format_name} (confidence: {confidence:.2f})"
        )

        if confidence < self.confidence_threshold:
            self.logger.warning(
                f"Low confidence format detection: {confidence:.2f} < {self.confidence_threshold}"
            )
