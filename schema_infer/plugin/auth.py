...
class AuthenticationManager:
        ...
        if security_protocol == "SASL_SSL":
            ...
            if self.config.kafka.ssl_certificate_location:
                config["ssl.certificate.location"] = (
                    self.config.kafka.ssl_certificate_location
                )
    ...
    def _configure_cloud_sr_auth(self) -> Dict[str, Any]:
        ...
        api_key = (
            self.config.schema_registry.cloud_api_key or self.config.kafka.cloud_api_key
        )
        api_secret = (
            self.config.schema_registry.cloud_api_secret
    ...
    def _configure_platform_sr_auth(self) -> Dict[str, Any]:
        ...
        if (
            self.config.schema_registry.username
            and self.config.schema_registry.password
        ):
        ...
        if self.config.schema_registry.ssl_certificate_location:
            config["ssl_certificate_location"] = (
                self.config.schema_registry.ssl_certificate_location
            )
    ...
    def _get_cloud_api_key(self) -> Optional[str]:
        ...
        if (
            hasattr(self.config.kafka, "cloud_api_key")
            and self.config.kafka.cloud_api_key
        ):
            return self.config.kafka.cloud_api_key

        # Check Schema Registry config as fallback
        if (
            hasattr(self.config.schema_registry, "cloud_api_key")
            and self.config.schema_registry.cloud_api_key
        ):
    ...
    def _get_cloud_api_secret(self) -> Optional[str]:
        ...
        # Check Kafka config first
        if (
            hasattr(self.config.kafka, "cloud_api_secret")
            and self.config.kafka.cloud_api_secret
        ):
            return self.config.kafka.cloud_api_secret

        # Check Schema Registry config as fallback
        if (
            hasattr(self.config.schema_registry, "cloud_api_secret")
            and self.config.schema_registry.cloud_api_secret
        ):
