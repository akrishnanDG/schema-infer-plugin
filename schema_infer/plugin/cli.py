...
@click.option(
    "--topic-pattern",
    default=".*",
    help="Regex pattern to match topics (default: all topics)",
)
@click.option(
    "--format", "-f",
    type=click.Choice(["avro", "protobuf", "json-schema"]),
...
@click.option(
    "--register",
    is_flag=True,
    help="Register schemas to Schema Registry",
)
@click.option(
    "--context",
...
@click.option(
    ...
    type=int,
    default=50,
    help="Maximum messages to sample per topic (default: 50)",
)
@click.option(
    "--exclude-internal",
    is_flag=True,
...
def watch(
    ...
    """
    Watch for new Kafka topics and automatically infer schemas.

    Continuously monitors the cluster for new topics, infers schemas,
    and optionally registers them to Schema Registry. Only processes
    topics that haven't been seen before.

    \b
    EXAMPLES:

    \b
    # Watch all topics and register Avro schemas
    schema-infer --config config.yaml watch --register --context production

    \b
    # Watch with custom interval and output
    ...
    click.echo(f"Watching for new topics (interval: {interval}s, format: {format})")
    if register:
        sr_context = f" under context '{context}'" if context else ""
        click.echo(f"  Registering schemas to Schema Registry{sr_context}")
    click.echo(f"  Output: {output_dir}/")
    click.echo(f"  Pattern: {topic_pattern}")
    ...
    try:
        while True:
            ...
            try:
                ...
                current_topics = discovery.discover_topics(
                    topic_pattern=topic_pattern,
                    exclude_internal=exclude_internal
                )

                new_topics = [t for t in current_topics if t not in processed_topics]

                if not new_topics:
                    click.echo(f"[{timestamp}] Cycle {cycle}: No new topics ({len(current_topics)} total, {len(processed_topics)} processed)")
                else:
                    click.echo(f"[{timestamp}] Cycle {cycle}: Found {len(new_topics)} new topic(s)")

                    # Process new topics
                    with OptimisticProcessor(config) as processor:
                        for topic_name in new_topics:
                            try:
                                click.echo(f"  Processing {topic_name}...", nl=False)

                                messages = processor.read_messages_shared_consumer(topic_name, max_messages, 30)

                                if not messages:
                                    click.echo(f" skipped (empty)")
                                ...
                                if not schema_dict:
                                    click.echo(f" skipped (no schema)")
                                    processed_topics.add(topic_name)
                                    continue
                                ...
                                if register and registry:
                                    try:
                                        from ..utils.validators import validate_generated_schema
                                        is_valid, err = validate_generated_schema(schema_content, format)
                                        if not is_valid:
                                            click.echo(f" saved, registration skipped (invalid: {err})")
                                            processed_topics.add(topic_name)
                                            continue

                                        schema_id = registry.register_schema(topic_name, schema_content, format)
                                        click.echo(f" saved + registered (ID: {schema_id})")
                                    except Exception as e:
                                        click.echo(f" saved, registration failed: {e}")
                                else:
                                    click.echo(f" saved")
                ...
                click.echo(f"[{timestamp}] Cycle {cycle}: Error - {e}")
