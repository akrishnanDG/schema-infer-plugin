...
class OptimisticProcessor:
    ...
    def _create_shared_consumer(self) -> Consumer:
        ...
        if hasattr(self.config.kafka, "api_version_request"):
            consumer_config["api.version.request"] = (
                self.config.kafka.api_version_request
            )
        if hasattr(self.config.kafka, "api_version_fallback_ms"):
            consumer_config["api.version.fallback.ms"] = (
                self.config.kafka.api_version_fallback_ms
            )
    ...
    def read_topics_parallel(
        ...
        def _reader_worker(topics_chunk: List[str]) -> None:
            ...
            try:
                for topic_name in topics_chunk:
                    try:
                        msgs = self._read_single_topic(
                            consumer, topic_name, max_messages, timeout
                        )
                        if msgs:
                            with lock:
        ...
        with ThreadPoolExecutor(max_workers=num_readers) as executor:
            futures = [
                executor.submit(_reader_worker, chunk) for chunk in chunks if chunk
            ]
    ...
    def _read_messages_from_assigned_partitions(
        self, consumer, partitions, max_messages: int, timeout: int, topic_name: str
    ) -> List[Tuple[Optional[bytes], bytes]]:
        """Read messages from manually assigned partitions."""

        messages = []
        self.logger.info(
            f"Reading messages from {len(partitions)} manually assigned partitions"
        )
        ...
        for partition in partitions:
            try:
                ...
                self.logger.info(
                    f"Getting watermark offsets for partition {partition.partition}"
                )
                low, high = consumer.get_watermark_offsets(partition, timeout=10.0)
                self.logger.info(
                    f"Partition {partition.partition} offsets: low={low}, high={high}"
                )
                ...
                if high > low:
                    ...
                    self.logger.info(
                        f"Seeking to offset {target_offset} on partition {partition.partition}"
                    )
                    consumer.seek(
                        confluent_kafka.TopicPartition(
                            topic_name, partition.partition, target_offset
                        )
                    )

                    # Batch poll for messages
                    self.logger.info(
                        f"Starting batch poll for partition {partition.partition}"
                    )
                    partition_messages = self._batch_poll_messages(
                        consumer, max_messages, timeout, topic_name
                    )
                    self.logger.info(
                        f"Read {len(partition_messages)} messages from partition {partition.partition}"
                    )
        ...
        self.logger.info(
            f"Total messages read from manually assigned partitions: {len(messages)}"
        )
        return messages[:max_messages]
    ...
    def _read_messages_from_partitions_parallel(
        self, consumer, topic_name: str, max_messages: int, timeout: int
    ) -> List[Tuple[Optional[bytes], bytes]]:
            ...
            for partition in assignment:
                try:
                    ...
                    if high > low:
                        ...
                        consumer.seek(
                            confluent_kafka.TopicPartition(
                                topic_name, partition.partition, start_offset
                            )
                        )
    ...
    def _do_quick_topic_check(self, topic_name: str) -> Tuple[bool, str]:
        ...
        try:
            ...
            try:
                ...
                while time.time() - start_time < 3:  # 3 second max
                    ...
                    if msg.error():
                        ...
                        if "offset" in error_str and "out of range" in error_str:
                            return (
                                False,
                                "Topic has messages but they may have expired due to retention policy",
                            )
                ...
                return (
                    True,
                    "Quick check inconclusive - proceeding with full processing",
                )
            ...
            return (
                True,
                "Unable to check topic state - proceeding with full processing",
            )  # Assume topic has messages if check fails
        ...
        try:
            ...
            try:
                ...
                if total_partitions > max_partitions_to_check:
                    ...
                    partitions_to_check = [
                        assignment[i] for i in range(0, total_partitions, step)
                    ][:max_partitions_to_check]
                    self.logger.info(
                        f"Topic has {total_partitions} partitions, sampling {len(partitions_to_check)} partitions"
                    )

                for partition in partitions_to_check:
                    try:
                        low, high = consumer.get_watermark_offsets(
                            partition, timeout=10.0
                        )
                        ...
                        self.logger.debug(
                            f"Failed to get offsets for partition {partition}: {e}"
                        )
                ...
                if total_messages == 0:
                    if total_partitions > max_partitions_to_check:
                        return (
                            True,
                            f"Topic is truly empty (sampled {len(partitions_to_check)} of {total_partitions} partitions)",
                        )
                    else:
                        return (
                            True,
                            "Topic is truly empty (no messages in any partition)",
                        )
                    ...
                    if total_partitions > max_partitions_to_check:
                        ...
                        return (
                            True,
                            "Topic is truly empty (all partitions have same beginning and end offsets)",
                        )
                else:
                    if total_partitions > max_partitions_to_check:
                        return (
                            False,
                            f"Topic has approximately {total_messages} messages across {total_partitions} partitions (sampled {len(partitions_to_check)} partitions)",
                        )
                    else:
                        return (
                            False,
                            f"Topic has {total_messages} messages across {total_partitions} partitions",
                        )
            ...
            return (
                False,
                "Unable to check topic offsets - proceeding with full processing",
            )
    ...
    def read_latest_messages(
        ...
        if self.config.performance.verbose_logging:
            self.logger.info(
                f"Reading latest {max_messages} messages from topic: {topic_name}"
            )
        start_time = time.time()
        ...
        try:
            ...
            self.logger.debug(f"Offset check failed: {e}")
            print(
                f"⚠️  Unable to determine topic state - proceeding with message reading"
            )
        ...
        try:
            ...
            if result:
                ...
                if self.config.performance.verbose_logging:
                    self.logger.info(
                        f"Successfully read {len(result)} messages using optimized strategy in {elapsed_time:.2f}s"
                    )
                # Only show progress output if verbose logging is enabled or progress bars are disabled
                if (
                    self.config.performance.verbose_logging
                    or not self.config.performance.show_progress
                ):
        ...
        try:
            ...
            if result:
                ...
                if self.config.performance.verbose_logging:
                    self.logger.info(
                        f"Successfully read {len(result)} messages using fallback strategy in {elapsed_time:.2f}s"
                    )
                # Only show progress output if verbose logging is enabled or progress bars are disabled
                if (
                    self.config.performance.verbose_logging
    ...
    def read_messages_shared_consumer(
        self, topic_name: str, max_messages: int, timeout: int
    ) -> List[Tuple[Optional[bytes], bytes]]:
        ...
        """
        with self._consumer_lock:
            return self._read_messages_shared_consumer_locked(
                topic_name, max_messages, timeout
            )

    def _read_messages_shared_consumer_locked(
        self, topic_name: str, max_messages: int, timeout: int
    ) -> List[Tuple[Optional[bytes], bytes]]:
        ...
        try:
            ...
            if not topic_metadata or not topic_metadata.partitions:
                if self.config.performance.verbose_logging:
                    self.logger.warning(
                        f"Topic {topic_name} not found or has no partitions"
                    )
                return []
            ...
            for partition in partitions:
                try:
                    ...
                    if self.config.performance.verbose_logging:
                        self.logger.debug(
                            f"Failed to seek partition {partition.partition}: {e}"
                        )
                    continue
            ...
            while (
                ...
                and time.time() - poll_start < timeout
                ...
                if msg is None:
                    ...
                    current_limit = (
                        max_empty_polls_streaming
                        if got_first_message
                        else max_empty_polls_initial
                    )
                ...
                if msg.error():
                    if msg.error().code() == ConfluentKafkaError._PARTITION_EOF:
                        ...
                        msg.error().code() == ConfluentKafkaError._UNKNOWN_TOPIC_OR_PART
                    ):
                        break
                    else:
                        continue
            ...
            if self.config.performance.verbose_logging:
                self.logger.debug(
                    f"Shared consumer reading failed for {topic_name}: {e}"
                )
            return []
        finally:
    ...
    def _strategy_optimized(
        ...
        try:
            ...
            while time.time() - start_time < assignment_timeout:
                ...
                if partitions:
                    self.logger.info(
                        f"Got {len(partitions)} partitions assigned: {[p.partition for p in partitions]}"
                    )
                    break
                consumer.poll(0.5)
            ...
            if not partitions:
                ...
                try:
                    ...
                    if topic_metadata:
                        ...
                        if partitions:
                            self.logger.info(
                                f"Manually assigning {len(partitions)} partitions: {[p.partition for p in partitions]}"
                            )
                            consumer.assign(partitions)
                            ...
                            if assigned:
                                self.logger.info(
                                    f"Manual assignment successful: {[p.partition for p in assigned]}"
                                )
                                # Continue with message reading
                                return self._read_messages_from_assigned_partitions(
                                    consumer,
                                    partitions,
                                    max_messages,
                                    timeout,
            ...
            messages_per_partition = (
                max_messages // len(partitions) if partitions else max_messages
            )
            ...
            for partition in partitions:
                try:
                    partition_messages = self._read_partition_optimized(
                        consumer, partition, messages_per_partition, timeout, topic_name
                    )
                    if partition_messages:
                        all_messages.extend(partition_messages)
                        if self.config.performance.verbose_logging:
                            self.logger.info(
                                f"Read {len(partition_messages)} messages from partition {partition.partition}, total: {len(all_messages)}"
                            )
                ...
                for partition in partitions:
                    try:
                        ...
                        if high > low:
                            # Calculate smart offset - read from end but not too far back
                            target_offset = max(
                                low, high - min(messages_per_partition * 2, 5000)
                            )  # Read 2x per partition or 5k, whichever is smaller
                            if self.config.performance.verbose_logging:
                                self.logger.info(
                                    f"Seeking to offset {target_offset} on partition {partition.partition}"
                                )
                            ...
                            partition_messages = self._batch_poll_messages(
                                consumer, messages_per_partition, timeout, topic_name
                            )
                            if partition_messages:
                                all_messages.extend(partition_messages)
                                if self.config.performance.verbose_logging:
                                ...
                                if len(all_messages) >= max_messages:
                                    if self.config.performance.verbose_logging:
                                        self.logger.info(
                                            f"Collected {len(all_messages)} messages from all partitions, returning early"
                                        )
                                    progress_bar.close()
                                    return all_messages[:max_messages]
                        ...
                        self.logger.debug(
                            f"Error reading from partition {partition}: {e}"
                        )
                        continue
            ...
            if all_messages:
                self.logger.info(
                    f"Collected {len(all_messages)} messages from all partitions"
                )
                progress_bar.close()
                return all_messages[:max_messages]
            ...
            if len(all_messages) < max_messages:
                self.logger.info(
                    f"Only got {len(all_messages)} messages from end offsets, trying beginning offsets for more messages"
                )
                for partition in partitions:
                    try:
                        low, high = consumer.get_watermark_offsets(
                        ...
                        if high > low:
                            self.logger.info(
                                f"Seeking to beginning offset {low} on partition {partition.partition}"
                            )
                            consumer.seek(
                                confluent_kafka.TopicPartition(
                                    topic_name, partition.partition, low
                                )
                            )

                            # Read additional messages from beginning
                            additional_messages = self._batch_poll_messages(
                                consumer,
                            ...
                            if additional_messages:
                                ...
                                self.logger.info(
                                    f"Added {len(additional_messages)} messages from beginning of partition {partition.partition}, total: {len(all_messages)}"
                                )
                                if len(all_messages) >= max_messages:
                                    break
                        ...
                        self.logger.debug(
                            f"Error reading from beginning offset for partition {partition}: {e}"
                        )
                        continue
    ...
    def _batch_poll_messages(
        self, consumer, max_messages: int, timeout: int, topic_name: str
    ) -> List[Tuple[Optional[bytes], bytes]]:
        ...
        while (
            ...
            try:
                ...
                if valid_messages >= max_messages:
                    if self.config.performance.verbose_logging:
                        self.logger.info(
                            f"Found {valid_messages} valid messages, stopping early"
                        )
                    break
        ...
        if self.config.performance.verbose_logging:
            self.logger.info(
                f"Batch polling completed: {valid_messages} valid messages from {batch_count} batches (target: {max_messages})"
            )
        if valid_messages < max_messages:
    ...
    def _read_partition_optimized(
        self,
        consumer,
        partition,
        messages_per_partition: int,
        timeout: int,
        topic_name: str,
    ) -> List[Tuple[Optional[bytes], bytes]]:
        ...
        try:
            ...
            if high > low:
                ...
                # Batch poll for messages from this partition
                return self._batch_poll_messages(
                    consumer, messages_per_partition, timeout, topic_name
                )
        except Exception as e:
            self.logger.debug(f"Error reading from partition {partition}: {e}")
    ...
    def _strategy_simple_fallback(
        ...
        try:
            ...
            start_time = time.time()
            self.logger.info(
                f"Fallback strategy: Waiting for partition assignment for topic: {topic_name}"
            )
            while time.time() - start_time < assignment_timeout:
                partitions = consumer.assignment()
                if partitions:
                    self.logger.info(
                        f"Fallback strategy: Got {len(partitions)} partitions assigned: {[p.partition for p in partitions]}"
                    )
                    break
                consumer.poll(0.5)
            ...
            if not partitions:
                ...
                try:
                    ...
                    if topic_metadata:
                        self.logger.info(
                            f"Fallback strategy: Topic metadata found: {topic_metadata}"
                        )
                    else:
                        self.logger.warning(
                            f"Fallback strategy: Topic {topic_name} not found in metadata"
                        )
                except Exception as e:
                    self.logger.warning(
                        f"Fallback strategy: Failed to get topic metadata: {e}"
                    )
                return []
                ...
                try:
                    ...
                    if high > low:
                        ...
                        for sample_offset in sample_points:
                            ...
                            sample_messages = self._batch_poll_messages(
                                consumer, max_messages // 4, timeout // 4, topic_name
                            )
