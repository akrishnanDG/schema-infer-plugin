...
class AvroGenerator(BaseSchemaGenerator):
    ...
    def generate(self, schema: InferredSchema) -> str:
        ...
        avro_schema = {
            ...
            or f"Auto-generated Avro schema for {schema.name}",
            "fields": [],
        }
    ...
    def _build_nested_avro_fields(
        ...
        for field_name, field in top_level_fields.items():
            if field_name in array_child_fields and array_child_fields[field_name]:
                ...
                item_fields = self._build_nested_avro_record_fields(
                    array_child_fields[field_name]
                )
                avro_field = {
                    ...
                    "type": {
                        ...
                        "items": {
                            "type": "record",
                            "name": self._unique_record_name(
                                f"{self._record_prefix}_{field_name}_item"
                            ),
                            "fields": item_fields,
                        },
                    },
            ...
            if top_level not in top_level_fields:
                nested_record = {
                    ...
                    "type": {
                        "type": "record",
                        "name": self._unique_record_name(
                            f"{self._record_prefix}_{top_level}_record"
                        ),
                        "fields": self._build_nested_avro_record_fields(
                            nested_list, top_level
                        ),
                    },
                ...
                avro_fields.append(
                    {
                        ...
                        "type": {
                            ...
                            "items": {
                                "type": "record",
                                "name": self._unique_record_name(
                                    f"{self._record_prefix}_{array_parent}_item"
                                ),
                                "fields": item_fields,
                            },
                        },
    ...
    def _build_nested_avro_record_fields(
        self, nested_fields: List[tuple], parent_path: str = ""
    ) -> List[Dict[str, Any]]:
        ...
        for field_name, field_list in groups.items():
            ...
            nested_entries = [
                (path, field) for path, field in field_list if path is not None
            ]
            ...
            if leaf_entries and not nested_entries:
                ...
                nested_record = {
                    ...
                    "type": {
                        "type": "record",
                        "name": self._unique_record_name(
                            f"{self._record_prefix}_{full_path}_record"
                        ),
                        "fields": self._build_nested_avro_record_fields(
                            nested_entries, full_path
                        ),
                    },
...
class ProtobufGenerator(BaseSchemaGenerator):
    ...
    def generate(self, schema: InferredSchema) -> str:
        ...
        # Add package
        if schema.namespace:
            package_name = self._sanitize_protobuf_name(
                schema.namespace.replace(".", "_").lower()
            )
        ...
        field_number = 1
        field_number = self._add_protobuf_fields(
            lines, nested_structure, field_number, indent="  "
        )
    ...
    def _build_nested_protobuf_structure(
        self, fields: List[SchemaField]
    ) -> Dict[str, Any]:
        """Build nested message structure from flat field names."""
        structure = {"top_level_fields": {}, "nested_messages": {}}
...
class JSONSchemaGenerator(BaseSchemaGenerator):
    ...
    def generate(self, schema: InferredSchema) -> str:
        ...
        json_schema = {
            ...
            or f"Auto-generated JSON Schema for {schema.name}",
            "type": "object",
            "properties": {},
            "required": [],
    ...
    def _convert_type_to_json_schema(self, field_type: FieldType) -> Dict[str, Any]:
        ...
        if field_type.array:
            array_schema = {"type": "array", "items": {"type": base_type}}
