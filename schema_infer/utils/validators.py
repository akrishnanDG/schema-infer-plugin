...
def validate_generated_schema(schema_content: str, schema_format: str) -> Tuple[bool, str]:
    """
    ...
    if schema.get("type") == "record":
        ...
        for i, field in enumerate(schema["fields"]):
            ...
            if isinstance(field_type, str):
                valid_types = {"null", "boolean", "int", "long", "float", "double", "bytes", "string"}
                if field_type not in valid_types:
                    return False, f"Avro field '{field['name']}' has invalid type '{field_type}'. Must be a primitive type or a {{\"type\": ...}} definition"
...
def _validate_protobuf_schema(schema_content: str) -> Tuple[bool, str]:
    ...
    if 'syntax = "proto3";' not in schema_content and "syntax = 'proto3';" not in schema_content:
    ...
    for i, line in enumerate(lines, 1):
        ...
        field_match = re.match(r'\s*(?:repeated\s+)?(?:\w+)\s+(\S+)\s*=\s*\d+', stripped)
        if field_match:
